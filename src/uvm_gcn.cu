...
//layer-1 full-graph aggregation
#pragma omp parallel for 
for(int mype_node = 0; mype_node < num_GPUs; mype_node++)
{
    cudaSetDevice(mype_node);
    ...
    dense_in2hidden_uvm* dp1 = new dense_in2hidden_uvm("d-1", d_input[mype_node], d_hidden, mype_node, numNodes, dim, hiddenSize);
    dense_hidden2out_uvm* dp2 = new dense_hidden2out_uvm("d-2", dsp_out, d_hidden, mype_node, numNodes, hiddenSize, outdim);
    ...
    SAG_full_UVM_updated(dsp_out, d_hidden,
                         d_row_ptr[mype_node], d_col_ind[mype_node],
                         hiddenSize, num_GPUs,
                         mype_node, numNodes);

    dense_hidden_forward_uvm(dp2);
    SAG_partial_UVM_updated(d_output, d_hidden,
                            d_row_ptr[mype_node], d_col_ind[mype_node],
